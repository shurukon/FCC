...
const { releaseAssetUrl, releasePageUrl, releaseTag } = require("./github");
const { cacheRoot, versionDir, appDir, exePath, downloadTmpZipPath } = require("./paths");

const MAX_REDIRECTS = 5;

/** Escape a path for safe interpolation inside a PowerShell single-quoted
 * string literal - doubling an embedded `'` is PowerShell's own escape for
 * it. Unlikely to ever matter (Windows paths rarely contain a quote), but
  ...
  try {
    ...
    throw new Error(
      `${err.message}\n\n` +
        ...
        `(built by .github/workflows/build-windows.yml on a "${releaseTag(version)}" tag push).`,
    );
  }
