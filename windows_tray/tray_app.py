...
def _bundle_dir() -> Path:
    """Return the directory bundled assets (app.ico, etc.) live under.

    When frozen, PyInstaller's bootloader can flatten the *main* script's
    own ``__file__`` to the bundle root - unlike regular imported modules,
    which keep their real package path (that's why api/admin_routes.py's
    own ``Path(__file__).parent``-based static-file lookup is fine as-is,
    but this file, being the entry script, can't rely on the same trick).
    ``sys._MEIPASS`` is always the right base for bundled data files
    regardless of onefile/onedir internals - see build.spec's ``datas``,
    which places app.ico at ``windows_tray/app.ico`` under exactly that root.
    """
...
def _load_icon_image():
    ...
    ico_path = _bundle_dir() / "windows_tray" / "app.ico"
    if ico_path.is_file():
