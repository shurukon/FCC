...
def _pooled_credential_field_specs() -> tuple[dict[str, Any], ...]:
    ...
    # Per-provider overrides for the generic pooled-slot description below -
    # currently just Cloudflare, whose pooled token needs a matching Account
    # ID slot (see _cloudflare_account_field_specs) to actually be usable.
    pooled_description_overrides = {
        "CLOUDFLARE_API_TOKEN": (
            "Token for an additional Cloudflare account. Also set the "
            "matching \"Cloudflare Account ID #N\" field below for this "
            "same slot number - a token alone isn't enough to use another "
            "account, since the account id is part of the request URL."
        ),
    }
    ...
    for descriptor in PROVIDER_CATALOG.values():
        ...
        pooled_description = pooled_description_overrides.get(
            descriptor.credential_env,
            ...
            specs.append(
                {
                    ...
                    "secret": True,
                    "advanced": True,
                    "description": pooled_description,
                }
            )
    return tuple(specs)
...
def _cloudflare_account_field_specs() -> tuple[dict[str, Any], ...]:
    ...
    extra_slots = pooled_env_keys("CLOUDFLARE_ACCOUNT_ID")[1:_UI_POOLED_KEY_SLOTS]
    for slot, env_key in enumerate(extra_slots, start=2):
        specs.append(
            {
                "key": env_key,
                "label": f"Cloudflare Account ID #{slot} (extra account)",
                "section_id": "providers",
                "advanced": True,
                "description": (
                    f"Account id paired with Cloudflare API Token #{slot} above - "
                    "both must be set together for that extra account to be used; "
                    "leave both blank if you only have one Cloudflare account."
                ),
            }
        )
    return tuple(specs)
