...
class AnthropicMessagesTransport(BaseProvider):
    ...
    def _effective_api_key(self, api_key: str | None) -> str:
        """Return the key to use for one request.

        ``api_key`` is the explicit pooled key threaded through the hot
        chat-streaming path (see ``_open_stream``). When it's ``None`` (the
        model-list path, or any other caller that doesn't rotate on its
        own), fall back to the pool's current key if pooling is configured,
        otherwise the single configured key - i.e. the original behavior.
        """
    ...
    def _stream_request_url(self, api_key: str | None) -> str:
        """Return the request target for one attempt.

        Defaults to the fixed relative path every provider except Cloudflare
        uses (httpx resolves it against ``self._client``'s base_url).
        Cloudflare's URL is account-scoped (``/accounts/{id}/ai/v1/...``),
        and which account is active depends on which pooled credential this
        attempt is using - see ``CloudflareProvider._stream_request_url``,
        which returns a full absolute URL instead, overriding the base_url
        join entirely (standard httpx behavior for an absolute URL).
        """
        return "/messages"
    ...
    async def _send_stream_request(
        ...
        request = self._client.build_request(
            "POST",
            self._stream_request_url(api_key),
            json=body,
    ...
    async def _validated_stream_send(
        self, body: dict, *, req_tag: str, api_key: str | None = None
    ) -> httpx.Response:
    ...
    async def _open_stream(self, body: dict, *, req_tag: str) -> httpx.Response:
        """Open the initial messages stream for a new request.

        Rotates through pooled keys on a failover-eligible error (bad/expired
        key, exhausted quota) before the caller ever sees a failure - see
        ``key_pool.call_with_key_rotation``. Used only for the primary
        request; recovery/continuation sub-requests in ``recovery.py`` call
        ``_validated_stream_send`` directly, which still resolves to the
        pool's current key via ``_effective_api_key`` - a key that already
        started this conversation successfully has proven itself, so those
        don't need a fresh rotation decision of their own.
        """
        from providers.runtime.key_pool import call_with_key_rotation

        async def attempt(key: str) -> httpx.Response:
            return await self._global_rate_limiter.execute_with_retry(
                self._validated_stream_send, body, req_tag=req_tag, api_key=key
            )

        if self._key_pool is None:
            return await attempt(self._api_key)
