...
def _create_cloudflare(config: ProviderConfig, settings: Settings) -> BaseProvider:
    ...
    from providers.runtime.key_pool import discover_paired_extra_values

    primary_account_id = settings.cloudflare_account_id
    account_id_by_token = {config.api_key.strip(): primary_account_id.strip()}
    for token, account_id in discover_paired_extra_values(
        "CLOUDFLARE_API_TOKEN",
        config.api_key,
        "CLOUDFLARE_ACCOUNT_ID",
        primary_account_id,
    ):
        account_id_by_token[token] = account_id

    return CloudflareProvider(
        config,
        account_id=primary_account_id,
        account_id_by_token=account_id_by_token,
    )
...
def _create_kilo(config: ProviderConfig, _settings: Settings) -> BaseProvider:
