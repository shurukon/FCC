...
from providers.runtime.key_pool import ProviderKeyPool
...
async def test_stream_uses_post_messages_path(
    ...
    assert mock_build.call_args.args[0] == "POST"
    assert mock_build.call_args.args[1] == f"{_BASE_URL}/messages"
    assert mock_build.call_args.kwargs["headers"]["Authorization"] == (
        "Bearer test-cloudflare-token"
    )


def test_second_account_resolves_to_its_own_url_not_the_primarys(
    cloudflare_config: ProviderConfig,
) -> None:
    """A pooled token from a second Cloudflare account must resolve to that
    account's own URL - reusing the primary account's URL with a foreign
    token would just get rejected by Cloudflare. This is the exact gap a
    token-only pool (no paired account id) would have missed.
    """
    other_account_id = "account-456"
    other_token = "second-account-token"
    with patch("httpx.AsyncClient"):
        provider = CloudflareProvider(
            cloudflare_config,
            account_id=_ACCOUNT_ID,
            account_id_by_token={
                "test-cloudflare-token": _ACCOUNT_ID,
                other_token: other_account_id,
            },
        )

    assert provider._account_id_for_key(None) == _ACCOUNT_ID
    assert provider._account_id_for_key(other_token) == other_account_id
    assert provider._stream_request_url(None) == f"{_BASE_URL}/messages"
    assert provider._stream_request_url(other_token) == (
        f"{CLOUDFLARE_AI_REST_ROOT}/accounts/{other_account_id}/ai/v1/messages"
    )
    # An unrecognized token (shouldn't happen in practice - every pooled
    # token is registered at construction time) falls back to the primary
    # account rather than raising, since a stream mid-request is not the
    # place to surface a KeyError.
    assert provider._account_id_for_key("unregistered-token") == _ACCOUNT_ID


@pytest.mark.asyncio
async def test_stream_rotates_to_second_account_url_on_auth_failure(
    cloudflare_config: ProviderConfig,
) -> None:
    """End-to-end: primary account's token fails with 401, the pool rotates
    to the second pooled token, and that retry must hit the *second
    account's* URL - proving the token and its account id travel together
    through an actual rotation, not just in isolation.
    """
    other_account_id = "account-456"
    other_token = "second-account-token"
    with patch("httpx.AsyncClient"):
    ...
    provider._key_pool = ProviderKeyPool(["test-cloudflare-token", other_token])
    ...
    assert [call.args[1] for call in mock_build.call_args_list] == [
        f"{_BASE_URL}/messages",
        f"{CLOUDFLARE_AI_REST_ROOT}/accounts/{other_account_id}/ai/v1/messages",
    ]
    assert [
        call.kwargs["headers"]["Authorization"] for call in mock_build.call_args_list
    ] == ["Bearer test-cloudflare-token", f"Bearer {other_token}"]
